...
import { FiArrowLeft, FiFilter, FiSearch, FiLoader } from 'react-icons/fi';
...
export default function HerbalProductsPage() {
  ...
  const [availability, setAvailability] = useState(''); // '' or 'instock'

  useEffect(() => {
    // Load herbal subcategories dynamically
    api.get('/categories', { params: { type: 'Herbal' } })
      .then(({ data }) => {
        if (data.success) {
  ...
  return (
    <div className="min-h-screen pb-24 md:pb-12 bg-gradient-to-b from-[#2B0008] via-[#1A0307] to-[#130305] text-cream">
      <Header />
      ...
      <main className="max-w-6xl mx-auto px-4 py-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-serif text-2xl md:text-3xl text-gold font-normal tracking-wide">Herbal Products</h1>
              <p className="text-cream/50 text-xs mt-1">Nourishing care crafted with pure ingredients</p>
            </div>
          </div>
          ...
          <div className="relative w-full md:w-72">
            <input
              type="text"
              placeholder="Search herbal items..."
              value={search}
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-[#290d11]/10 rounded-2xl border border-gold/15">
            <p className="text-cream/50 text-sm">No herbal products found matching your criteria.</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {products.map((p) => (
