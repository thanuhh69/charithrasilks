...
export default function AccessoriesPage() {
  ...
  useEffect(() => {
    // Load accessory subcategories dynamically
    api.get('/categories', { params: { type: 'Accessory' } })
      .then(({ data }) => {
        if (data.success) {
      ...
      <main className="max-w-6xl mx-auto px-4 py-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-serif text-2xl md:text-3xl text-gold font-normal tracking-wide">Accessories Collection</h1>
              <p className="text-cream/50 text-xs mt-1">Explore our range of premium accessory designs</p>
            </div>
          </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-[#290d11]/10 rounded-2xl border border-gold/15">
            <p className="text-cream/50 text-sm">No accessories found matching your criteria.</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {products.map((p) => (
