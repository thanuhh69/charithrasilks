...
import { FiPlus, FiEdit2, FiTrash2, FiSearch, FiLoader, FiSliders } from 'react-icons/fi';
...
function AccessoriesContent() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const { data } = await adminApi.get('/admin/products', { params: { search, type: 'Accessory' } });
      if (data.success) {
  ...
  const handleDelete = async (id, title) => {
    if (!confirm(`Delete Accessory "${title}"? This cannot be undone.`)) return;
    try {
      await adminApi.delete(`/admin/products/${id}`);
      toast.success('Accessory deleted successfully');
      load();
    } catch (err) {
      toast.error(err?.response?.data?.message || 'Could not delete accessory');
    }
  };
  ...
  const handleToggleActive = async (id) => {
    try {
      ...
      toast.error('Could not update accessory status');
    }
  };
      ...
      <div className="p-4 md:p-8 max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <h1 className="font-serif text-2xl text-cream font-semibold">
            {editId ? 'Edit Accessory' : 'Add New Accessory'}
          </h1>
          <button 
  ...
  return (
    <div className="p-4 md:p-8">
      <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
        <div>
          <h1 className="font-serif text-2xl text-cream font-semibold">Accessories Management</h1>
          <p className="text-cream/50 text-xs mt-1">Manage necklaces, bangles, and other fashion accessories</p>
        </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={7} className="p-6 text-center text-cream/50">No accessories found</td></tr>
            ) : (
              products.map((p) => (
                <tr key={p._id} className="border-b border-gold/10">
                  ...
                  <td className="p-4 text-cream/70">{p.material || '—'}</td>
      ...
      <div className="block md:hidden space-y-4">
        {loading ? (
          ...
          <p className="text-center text-cream/50 py-6">No accessories found</p>
        ) : (
          products.map((p) => (
